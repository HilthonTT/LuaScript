...
func numArg(name string, n int, args []Value) (int64, float64, bool, bool) {
	...
	if !ok {
		panic(errorf("bad argument #%d to '%s' (number expected, got %s)", n, name, TypeName(args[n-1])))
	}
	return i, f, isInt, ok
}
...
func stringArg(name string, n int, args []Value) string {
	...
	panic(errorf("bad argument #%d to '%s' (string expected, got %s)", n, name, TypeName(args[n-1])))
}
