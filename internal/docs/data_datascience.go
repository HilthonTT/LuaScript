...
print(a:reshape({4}):mean())`,
		...
		Entries: []Entry{
			...
			{Name: "concat", Kind: EntryFunction, Signature: "ndarray.concat(a, b [, axis]): ndarray",
				Summary: "Joins two 1-D arrays end to end (axis defaults to 1).",
				Detail:  `1-D only: concatenating higher-dimensional arrays raises.`},
...
local x = linalg.solve(a, {5, 11})`,
		...
		Entries: []Entry{
			{Name: "add", Kind: EntryFunction, Signature: "linalg.add(a, b): table", Summary: "Element-wise sum of two VECTORS of equal length."},
			{Name: "sub", Kind: EntryFunction, Signature: "linalg.sub(a, b): table", Summary: "Element-wise difference of two VECTORS of equal length."},
			{Name: "scale", Kind: EntryFunction, Signature: "linalg.scale(a, k): table", Summary: "Multiplies every element of the vector a by the scalar k."},
...
See examples/41_ml_module.lsc for a full run.`,
		...
		Entries: []Entry{
			...
			{Name: "load", Kind: EntryFunction, Signature: "ml.load(blob): net",
				Summary: "Restores a network from a string produced by net:marshal."},
			{Name: "load_file", Kind: EntryFunction, Signature: "ml.load_file(path): net",
				Summary: "Restores a network from a file written by net:save."},
