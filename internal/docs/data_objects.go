...
The method is :capture rather than :match because match is a keyword.`,
		...
		Entries: []Entry{
			...
			{Name: "capture", Kind: EntryMethod, Signature: "re:capture(s): ...string",
				Summary: "Returns the whole first match followed by its capture groups, or nil when there is none.",
				Detail:  "A group that did not participate in the match comes back as nil, so it can be distinguished from one that matched the empty string."},
			{Name: "find", Kind: EntryMethod, Signature: "re:find(s [, init]): number?, number?, ...string",
				Summary: "The start and end indices of the first match at or after init, followed by any capture groups. nil when there is no match.",
				Detail:  "Indices are 1-based and inclusive, matching string.find. init may be negative to count from the end."},
			{Name: "groups", Kind: EntryMethod, Signature: "re:groups(s): table?",
				Summary: "The named capture groups of the first match, keyed by name. nil when there is no match.",
				Detail:  "Unnamed groups are omitted, as are named groups that did not participate. Name a group with the RE2 syntax (?P<name>...)."},
			...
			{Name: "find_all_captures", Kind: EntryMethod, Signature: "re:find_all_captures(s [, n]): table",
				Summary: "Every match in s as an array of arrays, each holding the whole match followed by its groups. n caps the number returned."},
			...
			{Name: "replace_func", Kind: EntryMethod, Signature: "re:replace_func(s, fn): string",
				Summary: "Replaces every match with the result of fn(match, ...groups), so the replacement can be computed rather than rearranged."},
...
so a send racing a close reports "closed" instead of panicking.`,
	...
	},
	{
		...
		SeeAlso: []string{"std", "sort"},
		Entries: []Entry{
			{Name: "insert", Kind: EntryMethod, Signature: "b:insert(k, ...)",
				Summary: "Inserts one or more keys. Every argument is a key, not a value.",
				Detail: `The tree is a set of keys, not a key/value map: b:insert(50, 500)
stores two keys. All of them must match the tree's key type.`},
			{Name: "search", Kind: EntryMethod, Signature: "b:search(k): boolean", Summary: "Whether the key is present. A key of the wrong type is simply absent, not an error."},
			{Name: "remove", Kind: EntryMethod, Signature: "b:remove(k): boolean", Summary: "Removes one occurrence of a key, reporting whether it was there."},
...
print((a * 2):max())`,
		...
		Entries: []Entry{
			...
			{Name: "sinh", Kind: EntryMethod, Signature: "a:sinh(): ndarray", Summary: "Elementwise hyperbolic sine."},
			{Name: "cosh", Kind: EntryMethod, Signature: "a:cosh(): ndarray", Summary: "Elementwise hyperbolic cosine."},
			{Name: "asin", Kind: EntryMethod, Signature: "a:asin(): ndarray", Summary: "Elementwise arcsine."},
			{Name: "acos", Kind: EntryMethod, Signature: "a:acos(): ndarray", Summary: "Elementwise arccosine."},
			{Name: "atan", Kind: EntryMethod, Signature: "a:atan(): ndarray", Summary: "Elementwise arctangent."},
			{Name: "log2", Kind: EntryMethod, Signature: "a:log2(): ndarray", Summary: "Elementwise base-2 logarithm."},
			{Name: "log10", Kind: EntryMethod, Signature: "a:log10(): ndarray", Summary: "Elementwise base-10 logarithm."},
			{Name: "round", Kind: EntryMethod, Signature: "a:round(): ndarray", Summary: "Elementwise rounding to the nearest integer, halves away from zero."},
			{Name: "sign", Kind: EntryMethod, Signature: "a:sign(): ndarray", Summary: "Elementwise sign: -1, 0 or 1."},
			{Name: "clip", Kind: EntryMethod, Signature: "a:clip(lo, hi): ndarray", Summary: "Constrains every element to [lo, hi]."},
			{Name: "sort", Kind: EntryMethod, Signature: "a:sort(): ndarray",
				Summary: "The elements in ascending order, as a new flat array.",
				Detail:  "Flattened rather than sorted along an axis: choosing an axis silently would be a trap, so it is left to the caller to reshape."},
			{Name: "argsort", Kind: EntryMethod, Signature: "a:argsort(): ndarray",
				Summary: "The 1-based indices that would sort the array, as a new flat array.",
				Detail:  "Stable, so equal elements keep their relative order — which is what makes sorting by one key and then another behave as expected. Use it to reorder a second array in step with this one."},
			{Name: "median", Kind: EntryMethod, Signature: "a:median(): number", Summary: "The middle value, or the mean of the two middle values for an even count."},
			{Name: "cumsum", Kind: EntryMethod, Signature: "a:cumsum(): ndarray", Summary: "The running total, as a new flat array of the same length."},
			{Name: "diff", Kind: EntryMethod, Signature: "a:diff(): ndarray", Summary: "The differences between successive elements, one shorter than the input."},
			{Name: "any", Kind: EntryMethod, Signature: "a:any(): boolean", Summary: "Whether any element is non-zero."},
			{Name: "all", Kind: EntryMethod, Signature: "a:all(): boolean", Summary: "Whether every element is non-zero."},
			{Name: "nonzero", Kind: EntryMethod, Signature: "a:nonzero(): ndarray",
				Summary: "The 1-based flat indices of the non-zero elements.",
				Detail:  "Paired with a comparison — which yields 1s and 0s — this is how you find where a condition holds."},
			{Name: "count_nonzero", Kind: EntryMethod, Signature: "a:count_nonzero(): number", Summary: "How many elements are non-zero."},
...
print(df:select("name", "age")
	...
	},
	{
		...
		Entries: []Entry{
			...
			{Name: "marshal", Kind: EntryMethod, Signature: "net:marshal(): string",
				Summary: "Serialises the network to a JSON string, restorable with ml.load."},
			{Name: "save", Kind: EntryMethod, Signature: "net:save(path)",
				Summary: "Writes the serialised network to a file, restorable with ml.load_file."},
			{Name: "num_weights", Kind: EntryMethod, Signature: "net:num_weights(): number",
				Summary: "The total number of weights in the network."},
			{Name: "weights", Kind: EntryMethod, Signature: "net:weights(): table",
				Summary: "The weights as a nested table, one entry per layer."},
		},
	},
}
