...
import (
	...
	"github.com/hilthontt/luascript/native/ml"
)
...
func newTraining(layers []*ml.Layer) *internal {
...
func (t *OnlineTrainer) learn(n *ml.Neural, e Example, it int) {
	n.Forward(e.Input)
	t.calculateDeltas(n, e.Response)
	t.update(n, it)
}

func (t *OnlineTrainer) calculateDeltas(n *ml.Neural, ideal []float64) {
	for i, neuron := range n.Layers[len(n.Layers)-1].Neurons {
		t.deltas[len(n.Layers)-1][i] = ml.GetLoss(n.Config.Loss).Df(
			neuron.Value,
			ideal[i],
...
func (t *OnlineTrainer) update(n *ml.Neural, it int) {
	var idx int
